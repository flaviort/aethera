# Divindades de Aethera

Este arquivo centraliza a visão geral das entidades divinas de Aethera. Os detalhes completos de cada entidade devem ficar em suas respectivas pastas dentro de `divindades/`.

## Estrutura

- `divindades/dragoes-primordiais/` — Azharyon e Nythra
- `divindades/grandes-faiscas/` — as 12 Primordiais e Iraeth
- `divindades/divindades-menores/` — deuses locais e centelhas mais presentes no cotidiano
- `divindades/espiritos-e-entidades/` — espíritos, guardiões e presenças ligadas a lugares específicos

# Aethera — As 12 Divindades Primordiais

> *"Antes que houvesse pedra ou vento ou memória, houve o Sopro. E no Sopro havia Doze pensamentos ainda sem nome, cada um tão vasto quanto a eternidade que ainda não havia começado."*
> — Fragmento do Codex Aetheralis, Livro Primeiro

---

## Prefácio: A Natureza das Divindades Primordiais

As Divindades Primordiais de Aethera não são deuses no sentido que os mortais costumam imaginar — não são figuras trancadas em tronos de ouro, distribuindo graça e punição conforme o capricho. Elas são, cada uma, uma **força ontológica**: um princípio fundamental da realidade que tomou consciência de si mesmo. Gaiax não *governa* a terra — ela *é* a terra, em toda a sua extensão existencial. 

Isso torna as relações entre elas profundamente complexas. Elas não escolheram suas naturezas, assim como uma pedra não escolhe ser dura. Mas possuem, cada uma, uma consciência que transcende a natureza bruta — uma **vontade** que as permite agir além do simples imperativo cósmico. E é nesse espaço entre a natureza e a escolha que toda a tragédia e grandeza da cosmologia de Aethera reside.

Não existe "boa" ou "má" divindade. Existe apenas propósito, e a forma como cada ser — mortal ou divino — se relaciona com o seu.

---

## Origem

As divindades nasceram das faíscas geradas pela dança de **Azharyon** e **Nythra**. Nem todas as faíscas foram iguais: algumas deram origem às grandes forças primordiais do mundo, enquanto outras deram forma a deuses locais, espíritos, guardiões e entidades menores.

Não existem apenas 12 divindades em Aethera. Existem incontáveis presenças divinas. O que distingue as **12 Primordiais** não é bondade, maldade ou supremacia moral, mas o tamanho da faísca que as formou e o alcance do domínio que carregam.

## Hierarquia Das Faíscas

### As 12 Divindades Primordiais

São as maiores faíscas da criação. Cada uma recebeu um domínio de alcance universal e participa da continuidade da criação em Aethera.

### Divindades Menores

São faíscas menores, mas igualmente reais. Seus domínios costumam ser locais ou específicos: uma floresta, uma serra, um rio, uma cidade, uma prática ou um povo.

### Entidades Primordiais

Existem também seres antigos que não se encaixam perfeitamente como divindades. Algumas dessas entidades voltaram a se manifestar com mais intensidade desde o Eclipse Cósmico.

## As 12 Divindades Primordiais

| Nome | Domínio | Raças ou povos que mais veneram | Arquivo de detalhes |
|------|---------|----------------------------------|---------------------|
| **Thalor** | Mares e Oceanos | Sereanos e povos marítimos | `divindades/grandes-faiscas/thalor/thalor.md` |
| **Vulkar** | Fogo e Vulcões | Orcs e povos forjadores | `divindades/grandes-faiscas/vulkar/vulkar.md` |
| **Gaiax** | Terra | Anões | `divindades/grandes-faiscas/gaiax-montar/gaiax-montar.md` |
| **Montar** | Montanhas | Anões | `divindades/grandes-faiscas/gaiax-montar/gaiax-montar.md` |
| **Sylvael** | Florestas | Elfos e Sylvanos | `divindades/grandes-faiscas/sylvael/sylvael.md` |
| **Lumien** | Luz | Humanos e povos aéreos | `divindades/grandes-faiscas/lumien/lumien.md` |
| **Noctra** | Trevas | Povos subterrâneos e ordens ocultas | `divindades/grandes-faiscas/noctra/noctra.md` |
| **Aeris** | Ventos | Raças aéreas | `divindades/grandes-faiscas/aeris/aeris.md` |
| **Tempyr** | Tempestades | Povos aéreos e marinhos | `divindades/grandes-faiscas/tempyr/tempyr.md` |
| **Nymara** | Vida | Elfos e Humanos | `divindades/grandes-faiscas/nymara/nymara.md` |
| **Morveth** | Morte | Povos dos pântanos e subterrâneos | `divindades/grandes-faiscas/morveth/morveth.md` |
| **Chronar** | Tempo | Monges, cronistas e povos aéreos | `divindades/grandes-faiscas/chronar/chronar.md` |

## Caso Especial: Gaiax E Montar

Gaiax e Montar são as **Irmãs de Pedra**, gêmeas nascidas da mesma faísca que se partiu em dois no momento da criação. Por isso, seus detalhes permanecem juntos em:

- `divindades/grandes-faiscas/gaiax-montar/gaiax-montar.md`

Contos como o das irmãs, seus temperamentos, sinais, culto anão e demais aprofundamentos devem ficar nesse arquivo específico, e não neste índice central.

## O Mundo Atual

As divindades não aparecem mais diretamente no mundo. Desde o Grande Sono de Azharyon e Nythra, as Primordiais se comunicam por sinais, sonhos, presságios e raros intermediários.

Isso fez surgir, ao longo dos milênios:

- Igrejas e templos dedicados a cada divindade
- Ordens religiosas com hierarquias e dogmas próprios
- Cavaleiros sagrados e servos devotos
- Magos especializados em domínios divinos
- Guerras religiosas e disputas políticas
- Cultos proibidos e devoções clandestinas

## Regiões E Divindades Predominantes

| Região | Divindade predominante | Notas |
|--------|-------------------------|-------|
| Reinos Humanos | Lumien | Igreja da Luz e Cavaleiros Sagrados |
| Império Orc | Vulkar / Morveth | Rituais de guerra, forja e morte |
| Reinos Anões | Gaiax / Montar | Culto unificado de **Duath Eron** |
| Florestas Élficas | Sylvael / Nymara | Cultos da natureza e da vida |
| Cidades Subterrâneas | Noctra | Ordens arcanas e segredos antigos |
| Ilhas Flutuantes | Aeris / Chronar | Povos mais próximos da cosmogonia original |
| Oceanos | Thalor | Templos submersos e ritos marítimos |
| Pântanos | Morveth | Cultos funerários e espíritos antigos |
| Cultos Proibidos | **Iraeth** | Ilegais na maioria dos reinos |

## Notas Importantes

- **Trevas** e **Morte** não são domínios malignos por natureza.
- Não existem divindades boas ou más em essência; existem propósitos distintos.
- As raças aéreas preservam parte do entendimento mais completo sobre a ordem divina.
- O número exato de divindades menores é desconhecido.
- O Eclipse Cósmico intensificou presságios, silêncios e sinais em vários templos.
- Os templos de Chronar em especial são vistos com temor desde os eventos mais recentes.

## Iraeth, A 13ª

Iraeth não é reconhecida por instituições religiosas legítimas, mas continua sendo venerada por cultos secretos, especialmente entre os que se sentem esquecidos, abandonados ou traídos pelo destino.

Mais detalhes em:

- `divindades/grandes-faiscas/iraeth/iraeth.md`
